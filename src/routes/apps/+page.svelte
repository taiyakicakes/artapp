<script lang="ts">
	import { appLinksStore, addAppLink, deleteAppLink } from '$lib/stores/appLinks.svelte';
	...
	let sortAlpha = $state(true);

	const sortedLinks = $derived(() => {
		const list = [...appLinksStore.links];
		if (sortAlpha) list.sort((a, b) => a.label.localeCompare(b.label));
		return list;
	});
	...
	async function handleAdd() {
		...
		saving = true;
		try {
			await addAppLink(label.trim(), url.trim());
			label = '';
			url = '';
			showForm = false;
		} finally {
...
<div class="min-h-screen bg-gradient-to-br from-pink-50 via-rose-50 to-violet-50 px-4 pt-8 pb-6">
	<div class="mx-auto max-w-md flex flex-col gap-6">
		...
		<div class="flex items-center justify-between">
			<h1 class="text-2xl font-black text-pink-600">App Links</h1>
			<div class="flex gap-2">
				{#if appLinksStore.links.length > 0}
					<button
		...
		<div class="flex rounded-2xl bg-white p-1 shadow-sm">
			<button
				onclick={() => (sortAlpha = false)}
				class="flex-1 rounded-xl py-2 text-sm font-bold transition-colors {!sortAlpha
			...
			<div class="rounded-2xl bg-white p-4 shadow-md">
				<div class="flex flex-col gap-3">
					<input
						...
						placeholder="Label (e.g. Anime Expo)"
					...
					<input
						...
						bind:value={url}
						onkeydown={handleKeydown}
						placeholder="URL (e.g. animeexpo.com/apply)"
						class="rounded-xl border border-pink-200 px-3 py-2 text-sm font-medium text-gray-700 outline-none focus:border-pink-400 focus:ring-2 focus:ring-pink-100"
					/>
					<button
						onclick={handleAdd}
		...
		{#if appLinksStore.loading}
			<div class="flex justify-center py-8">
				<div class="h-8 w-8 animate-spin rounded-full border-4 border-pink-200 border-t-pink-500"></div>
			</div>
		{:else if appLinksStore.links.length === 0}
			<div class="flex flex-col items-center gap-3 py-16 text-center">
				<p class="font-bold text-pink-300">No app links yet</p>
				<p class="text-sm text-pink-200">Tap + to add your first one</p>
			</div>
		{:else}
